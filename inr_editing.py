...
def main(args=None):
    ...
    ######In order to save model (Bart#####)
    results_dir = os.path.join("results", conf.get("experiment_name", "experiment"))
    os.makedirs(results_dir, exist_ok=True)

    model_save_path = os.path.join(results_dir, f"model_INR_editing_{conf['train_args']['seed']}.pt")
    results_file = os.path.join(results_dir, f"model_INR_editing_{conf['train_args']['seed']}.yaml")

   #######End of Bart's code########

    for epoch in epoch_iter:

        #########Bart: ceep track of loss per epoch #########
        ...
        for i, batch in enumerate(train_loader):
            ...
            if (global_step + 1) % conf['train_args']['eval_every'] == 0:
                ...
                if conf["wandb"]:
                    ...
                    wandb.log(log)
        #########Bart: Print the avg loss per epoch#########
            Total_loss_epoch += loss.item()
            num_batches_epoch += 1
        avg_loss_epoch = Total_loss_epoch / num_batches_epoch
        print(f"Epoch {epoch} completed, loss: {avg_loss_epoch:.3f}")
        #########End of Bart's code#########

        
    #########Bart: Print the avg test loss after training#########
    print(f"Final Test Loss (MSE): {test_loss:.3f}")
    #########End of Bart's code#########

    
        #########In order to save results (Bart)#########
    # Save results to file
    results = {
        "best_validation": {
            "avg_loss": float(best_val_results["avg_loss"]) if best_val_results is not None else None,
        },
        "test_at_best_validation": {
            "avg_loss": float(best_test_results["avg_loss"]) if best_test_results is not None else None,
        },
