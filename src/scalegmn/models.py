...
class BaseScaleGMN(nn.Module):
    ...
    def forward(self, batch, w=None, b=None):
        #print(f"@@@ BaseGMN.forward START: batch.node2type shape = {batch.node2type.shape} BEFORE CONSTRUCT GRAPH")
        batch = self.construct_graph(batch)
        #print(f"@@@ BaseGMN.forward START: batch.node2type shape = {batch.node2type.shape} AFTER CONSTRUCT GRAPH")
...
class ScaleGMN(BaseScaleGMN):
    ...
    def forward(self, batch, w=None, b=None):
        # --- Add checks for node counts ---
        actual_nodes_in_batch = batch.num_nodes
        nodes_from_init = self.num_nodes
        # --- End checks ---
...
class ScaleGMN_GNN_fw(ScaleGMN_GNN):
    ...
    def forward(self, batch, num_nodes=None, pos_embed=None, pos_embed_edge=None):
        for i, layer in enumerate(self.fw_layers):
            ...
            # --- !!! FIX for sign_mask START !!! ---
            node_sign_mask = None
            if hasattr(batch, 'sign_mask') and batch.sign_mask is not None:
                # Check if it's a Batch object with the .batch attribute
                if hasattr(batch, 'batch') and batch.batch is not None:
                    # Original logic for Batch object (uses node->graph mapping)
                    try:
                         node_sign_mask = batch.sign_mask[batch.batch].unsqueeze(-1)
                    except IndexError as e:
                         print(f"Warning: IndexError during sign_mask broadcasting using batch.batch: {e}")
                         node_sign_mask = None
                         
                # Handle single Data object (lacks .batch attribute)
                elif batch.sign_mask.numel() == 1: # Check if it's a per-graph scalar/tensor(1)
                    num_nodes_in_graph = batch.num_nodes # Get node count for this graph
                    per_graph_sign_mask = batch.sign_mask.squeeze() # Get scalar boolean value
                    # Repeat the per-graph value for all nodes
                    node_sign_mask = per_graph_sign_mask.repeat(num_nodes_in_graph).unsqueeze(-1) # Shape [N, 1]
                    # print(f"@@@ Corrected node_sign_mask shape for single graph: {node_sign_mask.shape}") # Optional debug
                else:
                    # Fallback: sign_mask might already be per-node, just ensure correct dims
                    print(f"Warning: sign_mask found with unexpected shape {batch.sign_mask.shape}. Ensuring [N, 1].")
                    node_sign_mask = batch.sign_mask.view(-1, 1)

                # Final check on shape consistency
                if node_sign_mask is not None and node_sign_mask.shape[0] != x.shape[0]:
                     print(f"ERROR: Final node_sign_mask shape {node_sign_mask.shape[0]} doesn't match x shape {x.shape[0]}")
                     node_sign_mask = None # Prevent passing incorrect shape
            # --- !!! FIX for sign_mask END !!! ---
        ...
        if self.equivariant:
            ...
            if not self.only_last_layer:
                ...
                # --- FIX: Handle num_graphs for single Data object ---
                num_graphs = batch.num_graphs if hasattr(batch, 'num_graphs') else 1
                # --- END FIX ---
                node_features = x.reshape(num_graphs, num_nodes, x.shape[-1])  # change if processing varying architectures
                node_features = node_features[:, -self.last_layer_nodes:].flatten(1, 2)
                if pos_embed is not None:
                    pos_embed = pos_embed.reshape(num_graphs, num_nodes, pos_embed.shape[-1])
