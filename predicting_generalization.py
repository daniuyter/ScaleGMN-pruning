...
def main():
    ...
    for epoch in range(conf['train_args']['num_epochs']):
        ...
        for i, batch in enumerate(tqdm(train_loader)):
            ...
            print(pred_acc, 'predicted accuracy')
            print(gt_test_acc, 'g_t test acc')
    ...
    final_test_results = evaluate(net, test_loader, criterion, device)
    
    print(f"Final Test Results:")
    print(f"Test L1 Error: {final_test_results['avg_err']:.4f}")
    print(f"Test Loss: {final_test_results['avg_loss']:.4f}")
    print(f"Test R² Score: {final_test_results['rsq']:.4f}")
    print(f"Test Kendall's Tau: {final_test_results['tau']:.4f}")
    
    print("\nBest Validation Results:")
    if best_val_results is not None:
        print(f"Best Val L1 Error: {best_val_results['avg_err']:.4f}")
        print(f"Best Val Loss: {best_val_results['avg_loss']:.4f}")
        print(f"Best Val R² Score: {best_val_results['rsq']:.4f}")
        print(f"Best Val Kendall's Tau: {best_val_results['tau']:.4f}")
        
        print("\nTest Results at Best Validation:")
        print(f"Test L1 Error: {best_test_results['avg_err']:.4f}")
        print(f"Test Loss: {best_test_results['avg_loss']:.4f}")
        print(f"Test R² Score: {best_test_results['rsq']:.4f}")
        print(f"Test Kendall's Tau: {best_test_results['tau']:.4f}")
    
    # Save results to file
    results_dir = os.path.join("results", conf.get("experiment_name", "experiment"))
    os.makedirs(results_dir, exist_ok=True)
    
    results_file = os.path.join(results_dir, f"results_{conf['train_args']['seed']}.yaml")
    
    model_save_path = os.path.join(results_dir, f"model_seed_{conf['train_args']['seed']}.pt")
    try:
    ...
    results = {
        "final_test": {
            "l1_error": float(final_test_results['avg_err']),
            "loss": float(final_test_results['avg_loss']),
            "r2": float(final_test_results['rsq']),
            "kendall_tau": float(final_test_results['tau'])
        },
        "best_validation": {
            "l1_error": float(best_val_results['avg_err']) if best_val_results is not None else None,
            "loss": float(best_val_results['avg_loss']) if best_val_results is not None else None,
            "r2": float(best_val_results['rsq']) if best_val_results is not None else None,
            "kendall_tau": float(best_val_results['tau']) if best_val_results is not None else None,
        },
        "test_at_best_validation": {
            "l1_error": float(best_test_results['avg_err']) if best_test_results is not None else None,
            "loss": float(best_test_results['avg_loss']) if best_test_results is not None else None,
            "r2": float(best_test_results['rsq']) if best_test_results is not None else None,
            "kendall_tau": float(best_test_results['tau']) if best_test_results is not None else None,
        },
    ...
    # If using wandb, log final metrics
    if conf["wandb"]:
        wandb.run.summary.update({
            "final_test_l1_err": final_test_results['avg_err'],
            "final_test_loss": final_test_results['avg_loss'],
            "final_test_rsq": final_test_results['rsq'],
            "final_test_tau": final_test_results['tau'],
            "best_val_l1_err": best_val_results['avg_err'] if best_val_results is not None else None,
            "best_val_loss": best_val_results['avg_loss'] if best_val_results is not None else None,
            "best_val_rsq": best_val_results['rsq'] if best_val_results is not None else None,
            "best_val_tau": best_val_results['tau'] if best_val_results is not None else None,
        })
